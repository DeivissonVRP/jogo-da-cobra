...
HORIZONTE_DIA   = (80, 120, 175)   # cor das "montanhas" do horizonte de dia
HORIZONTE_NOITE = (15, 15, 40)     # cor das "montanhas" do horizonte de noite

TRENO_COR    = (30, 90, 190)     # cor do trenó/prancha em que o pinguim desliza
TRENO_BRILHO = (100, 160, 235)   # cor do contorno/brilho do trenó
...
def gerar_horizonte():
    # gera uma vez (por partida) os pontos de um horizonte em ziguezague, tipo silhueta de montanhas
    pontos = [(-10, CEU_ALTURA)]  # começa fora da tela à esquerda, na linha base do horizonte
    x = -10
    while x <= LARGURA + 10:
        # cada "pico" sobe um pouco acima da linha base (CEU_ALTURA), formando o ziguezague
        y = CEU_ALTURA - random.randint(4, 18)
        pontos.append((x, y))
        x += random.randint(18, 32)  # distância variável entre os picos
    pontos.append((LARGURA + 10, CEU_ALTURA))  # fecha o desenho fora da tela à direita
    return pontos


def desenhar_horizonte(tela, pontos_horizonte, fator_noite):
    # cor do horizonte também muda entre dia e noite, junto com o céu
    cor_horizonte = interpolar_cor(HORIZONTE_DIA, HORIZONTE_NOITE, fator_noite)
    # desenha o polígono fechado (picos + base), preenchendo a "faixa de montanhas"
    poligono = pontos_horizonte + [(LARGURA + 10, CEU_ALTURA + 6), (-10, CEU_ALTURA + 6)]
    pygame.draw.polygon(tela, cor_horizonte, poligono)
...
class Pinguim:
    ...
    def desenhar(self, tela, invertido):
        ...
        # trenó (desenhado primeiro, para ficar embaixo do corpo do pinguim)
        treno_rect = pygame.Rect(self.x - 6, self.y + self.altura - 8, self.largura + 12, 12)
        pygame.draw.rect(tela, TRENO_COR, treno_rect, border_radius=5)
        pygame.draw.rect(tela, TRENO_BRILHO, treno_rect, width=2, border_radius=5)
...
def rodar_jogo():
    ...
    horizonte = gerar_horizonte()  # picos do horizonte, gerados uma única vez por partida
    ...
    while rodando:
        ...
        if jogo_ativo:
            ...
            mover_nuvens(nuvens)
            fator_noite = desenhar_ceu(tela, nuvens, estrelas, tempo_ms)
            desenhar_horizonte(tela, horizonte, fator_noite)
            desenhar_chao(tela)
            for obstaculo in obstaculos:
